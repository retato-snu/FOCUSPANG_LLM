...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    elif "rubis" in dataset_name:
        chat_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.path.pardir,
                         os.path.pardir, os.path.pardir))
        return raw_datasets.RubisDataset(output_path, seed, local_rank, dataset_name, chat_path)
    elif "rubissft" in dataset_name:
        chat_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.path.pardir,
                         os.path.pardir, os.path.pardir))
        chat_path += '/data_sft'
        return raw_datasets.RubisSFTDataset(output_path, seed, local_rank, dataset_name, chat_path)
    elif "rubisrm" in dataset_name:
        chat_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.path.pardir,
                         os.path.pardir, os.path.pardir))
        chat_path += '/data_rm'
        return raw_datasets.RubisRMDataset(output_path, seed, local_rank, dataset_name, chat_path)
    elif "rubisppo" in dataset_name:
        chat_path = os.path.abspath(
            os.path.join(os.path.dirname(__file__), os.path.pardir,
                         os.path.pardir, os.path.pardir))
        chat_path += '/data_ppo'
        return raw_datasets.RubisPPODataset(output_path, seed, local_rank, dataset_name, chat_path)
    else:
        raise RuntimeError(
...
def create_prompt_dataset(local_rank,
    ...
    print("cache_found = True" + str(cache_found))
    buf_create_cache = torch.ByteTensor([not cache_found]).to(
        get_accelerator().current_device_name())
    torch.distributed.all_reduce(buf_create_cache)
    print("if is start")
    print("buf_create_cache" + str(buf_create_cache.item()))
