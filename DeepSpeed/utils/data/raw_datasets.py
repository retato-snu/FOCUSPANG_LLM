...
class RubisDataset(PromptRawDataset):
    ...
    def get_prompt(self, sample):
        if sample['prompt'] is not None:
            print(" Human: " + sample['prompt'] + " Assistant:")
            return " Human: " + sample['prompt'] + " Assistant:"
        return None

    # The chosen response should be in the format of: " " + actual_response_sentence
    def get_chosen(self, sample):
        if sample['ranking'] is not None and sample['completion{}'.format(sample['ranking'][0])] is not None:
            return " " + sample['completion{}'.format(sample['ranking'][0])]
        elif sample['completion'] is not None:
    ...
    # If the dataset does not have rejected response, return None
    def get_rejected(self, sample):
        if sample['ranking'] is not None and sample['completion{}'.format(sample['ranking'][2])] is not None:
            return " " + sample['completion{}'.format(sample['ranking'][2])]
        return None

    def get_prompt_and_chosen(self, sample):
        if sample['prompt'] is not None and sample['ranking'] is not None and sample['completion{}'.format(sample['ranking'][0])] is not None:
            print(" Human: " + sample['prompt'] + " Assistant:" + " " + sample['completion{}'.format(sample['ranking'][0])])
            return " Human: " + sample['prompt'] + " Assistant:" + " " + sample['completion{}'.format(sample['ranking'][0])]
        elif sample['prompt'] is not None and sample['completion'] is not None:
            print(" Human: " + sample['prompt'] + " Assistant:" + " " + sample['completion'])
            return " Human: " + sample['prompt'] + " Assistant:" + " " + sample['completion']
        return None

    def get_prompt_and_rejected(self, sample):
        if sample['prompt'] is not None and sample['ranking'] is not None and sample['completion{}'.format(sample['ranking'][2])] is not None:
            return " Human: " + sample['prompt'] + " Assistant:" + " " + sample['completion{}'.format(sample['ranking'][2])]
        return None
