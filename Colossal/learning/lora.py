...
class Embedding(nn.Embedding, LoraLayer):
    ...
    def get_delta_weight(self, adapter):
        return (
            transpose(
                self.lora_embedding_B[adapter] @ self.lora_embedding_A[adapter], True
            )
            * self.scaling[adapter]
        )
...
class Conv2d(nn.Conv2d, LoraLayer):
    ...
    def get_delta_weight(self, adapter):
        ...
        if self.weight.size()[2:4] == (1, 1):
            # conv2d 1x1
            return (
                self.lora_B[adapter].weight.squeeze(3).squeeze(2)
                @ self.lora_A[adapter].weight.squeeze(3).squeeze(2)
