...
class LowLevelZeroOptimizer(OptimizerWrapper):
    ...
    def __init__(
        ...
        for group_id, param_group in enumerate(self.optim.param_groups):
            ...
            master_param_current_rank = self._create_master_param_current_rank(
                group_params
            )

            self._master_param_groups_of_current_rank[
                group_id
    ...
    def _create_master_param_current_rank(self, param_list):
        ...
        for param in param_list:
            padding_size = (
                self._world_size - param.numel() % self._world_size
            ) % self._world_size
            ...
            with torch.no_grad():
                ...
                splited_params = padding_param.split(
                    padding_param.numel() // self._world_size
                )

                splited_param_current_rank = (
                    splited_params[self._local_rank].detach().float().to(device)
                )
                params_current_rank.append(splited_param_current_rank)
                self._param_store.link_master_and_working_param(
                    splited_param_current_rank, param
                )
            ...
            with torch.cuda.stream(stream):
                ...
                if not self._partition_grads:
                    ...
                    flat_grads_per_rank = flat_grads.split(
                        flat_grads.numel() // self._world_size
                    )
                    ...
                    for rank, grad_list in grad_in_bucket.items():
                        ...
                        for grad in grad_list:
                            ...
                            if (
                                ...
                                self._grad_store.append_gradients_by_param_id(
                                    grad, group_id, param_id
                                )
                            else:
                                self._grad_store.add_gradients_by_param_id(
                                    grad, rank, group_id, param_id
                                )

                else:
                    flat_grads_list = list(
                        flat_grads.split(len(flat_grads) // self._world_size)
                    )
                    recieved_grad = torch.zeros_like(flat_grads_list[0])
                    dist.reduce_scatter(
                        recieved_grad, flat_grads_list, group=self.dp_pg
                    )

                    if recieved_grad.dtype != grad_dtype:
                        recieved_grad = recieved_grad.to(grad_dtype)

                    grad_in_bucket_current_rank = self._bucket_store.get_grad()[
                        self._local_rank
                    ]
    ...
    def step(self, closure=None):
        ...
        for group_id in range(self.num_param_groups):
            ...
            for splited_param in master_params:
                working_param = self._param_store.master_to_working_param[
                    id(splited_param)
                ]
                ...
                if len(grads) > 0:
                    ...
                    grad = (
                        grads[grad_index]
                        .to(splited_param.dtype)
                        .to(splited_param.device)
                    )
            ...
            norm_group = compute_norm(
                gradients=working_grads, dp_group=self.dp_pg, tp_group=self.tp_pg
            )
        ...
        for group_id in range(self.num_param_groups):
            ...
            for idx, splited_param in enumerate(master_working_param):
                ...
                dist.all_gather(
                    all_splited_param, splited_param.cuda().to(dtype), group=self.dp_pg
                )
                working_param.data.copy_(
                    flatten(all_splited_param)[: working_param.numel()].reshape_as(
                        working_param
                    )
                )

            self.optim.param_groups[group_id][
                "params"
            ] = self._master_param_groups_of_current_rank[group_id]
        ...
        for param, state in self.optim.state.items():
            ...
            for k, v in state.items():
                if isinstance(v, torch.Tensor) and k != "step":
                    ...
                    gather_tensor = [
                        torch.zeros(v.shape, device="cuda", dtype=v.dtype)
                        for _ in range(self._world_size)
                    ]
                    dist.all_gather(gather_tensor, v.cuda(), group=self.dp_pg)
                    param_state = (
                        torch.stack(gather_tensor)
                        .view(-1)[: working_param.numel()]
                        .reshape_as(working_param)
                        .cpu()
                    )
    ...
    def load_state_dict(self, state_dict: Dict):
        ...
        for param_idx, state in zero_state_dict["state"].items():
            for k, v in state.items():
                if isinstance(v, torch.Tensor) and k != "step":
                    padding_size = (
                        self._world_size - v.numel() % self._world_size
                    ) % self._world_size
                    with torch.no_grad():
                        v = v.flatten()
                        if padding_size > 0:
                        ...
                        zero_state_dict["state"][param_idx][k] = (
                            v_list[self._local_rank].detach().clone()
                        )

        self.optim.load_state_dict(zero_state_dict)

    def state_dict_shard(
        self, max_shard_size: int = 1024
        ...
        for param_idx, states in local_states.items():
            ...
            if (
                ret_block_size + current_block_size > max_shard_size
                and len(ret_block) > 0
            ):
    ...
    def update_master_params(self, model: nn.Module) -> None:
        ...
        for p in model.parameters():
            ...
            if p_id in self._param_store.working_to_master_param:
                ...
                if padding_size > 0:
                    working_param = torch.nn.functional.pad(
                        working_param, [0, padding_size]
                    )
                master_param.copy_(
                    working_param.chunk(self._world_size)[self._local_rank]
                )
