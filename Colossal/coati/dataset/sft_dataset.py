...
def _preprocess(
    ...
    assert (
        sequences_token["attention_mask"].dim() == 2
...
class SFTDataset(Dataset):
    ...
    def __init__(
        self, dataset: Dict, tokenizer: PreTrainedTokenizer, max_length: int = 512
    ) -> None:
        super().__init__()
        self.input_ids = []
        ...
        if isinstance(tokenizer, ChatGLMTokenizer):
            ...
            self.input_ids, self.labels, self.attention_mask = _preprocess(
                sources, targets, tokenizer, max_length
            )
...
class SupervisedDataset(Dataset):
    ...
    def __init__(
        ...
        targets = [
            example["output"] + tokenizer.eos_token for example in list_data_dict
        ]
